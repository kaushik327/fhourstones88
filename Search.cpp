...
score Hash::transpose() {
  return he->biglock == lock   ? he->bigscore
         : he->newlock == lock ? he->newscore
                               : UNKNOWN;
}
  ...
  for (int i = 0; i < (WIDTH + 1) / 2; i++)
    for (int h = 0; h < HEIGHT1 / 2; h++)
      hist[HEIGHT1 * i + h] = hist[HEIGHT1 * (WIDTH - 1 - i) + HEIGHT - 1 - h] =
          hist[HEIGHT1 * i + HEIGHT - 1 - h] =
              hist[HEIGHT1 * (WIDTH - 1 - i) + h] =
...
int Book::loaded() const { return loadsize; }
int Book::added() const { return bookmap.size() - loadsize; }
...
score Search::dab(int depth, int alpha, int beta) {
  ...
  for (int i = 0; i < WIDTH; i++) {
    ...
    if (!game->islegal(newbrd))
      continue;
    game->makemove(i);
    score val = (score)(LOSSWIN -
                        (v = dab(depth - 1, LOSSWIN - beta, LOSSWIN - alpha)));
    game->backmove();
    if (v == UNKNOWN) {
      unknown = true;
      continue;
